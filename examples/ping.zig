...
//!
//! Note: uses Windows IP Helper (`IcmpSendEcho`). Other OS: compile error with hint.
...
pub fn main(init: std.process.Init) !void {
    if (builtin.os.tag != .windows) {
        std.debug.print("examples/ping currently supports Windows only (IcmpSendEcho).\n", .{});
    ...
    const dest = try resolveIpv4(io, host);
    ...
    while (i < count) : (i += 1) {
        sent += 1;
        const result = windowsIcmpEcho(dest, size, timeout_ms) catch |err| {
            std.debug.print("Request timed out. ({s})\n", .{@errorName(err)});
            if (i + 1 < count) Sleep(interval_ms);
            continue;
        };
        recv += 1;
...
fn resolveIpv4(io: Io, host: []const u8) ![4]u8 {
    _ = io;
